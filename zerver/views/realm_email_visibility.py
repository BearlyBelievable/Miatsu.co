...
def _violating_counts(
    ...
    too_open_values, too_closed_values = email_address_visibility_violations(
        visibility_max, visibility_min
    )

    if too_open_values:
        too_open_count = UserProfile.objects.filter(
            realm=realm, email_address_visibility__in=too_open_values, is_bot=False
        ).count()
    else:
        too_open_count = 0

    if too_closed_values:
        too_closed_count = UserProfile.objects.filter(
            realm=realm, email_address_visibility__in=too_closed_values, is_bot=False
        ).count()
    else:
        too_closed_count = 0

    return {"too_open_count": too_open_count, "too_closed_count": too_closed_count}
...
def _clamp_realm_default_if_needed(
    realm: Realm,
    too_open_values: list[int],
    too_closed_values: list[int],
    ...
    if current_value in too_open_values and visibility_max:
        do_set_realm_user_default_setting(
            realm_user_default, "email_address_visibility", visibility_max, acting_user=acting_user
        )
    elif current_value in too_closed_values and visibility_min:
        do_set_realm_user_default_setting(
...
def update_email_visibility_policy(
    ...
    *,
    ...
    _clamp_realm_default_if_needed(
        realm, too_open_values, too_closed_values, visibility_max, visibility_min, user_profile
    )
    if visibility_max:
        _create_and_enqueue_job(realm, too_open_values, visibility_max, counts["too_open_count"])
    if visibility_min:
        _create_and_enqueue_job(
            realm, too_closed_values, visibility_min, counts["too_closed_count"]
        )

    job_started = counts["too_open_count"] > 0 or counts["too_closed_count"] > 0
    event: dict[str, Any] = {
        "type": "realm",
        "op": "update_dict",
        ...
        "data": {
            "running": job_started,
            "total_violating_count": counts["too_open_count"] + counts["too_closed_count"],
        },
    }
