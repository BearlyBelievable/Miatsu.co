...
class TusdPreFinishTest(ZulipTestCase):
    ...
    def test_tusd_pre_finish_hook_audio_extension_fallback(self) -> None:
        # Many operating systems have no MIME type registered for
        # these formats, so the browser can report a generic
        # application/octet-stream type that falls back to guessing
        # from the extension.
        self.login("hamlet")
        hamlet = self.example_user("hamlet")
