...
export function handle_remediation_failed_event(): void {
    // The event itself only carries which field failed (see
    // server_events_dispatch.js); re-fetch status for the actual
    // counts, reusing the exact same display path the initial page
    // load already uses for a failure found there.
    void channel.get({
...
export function setup(): void {
    ...
    void channel.get({
        ...
        success(raw_data) {
            ...
            // The slider is only ever created here, once its real
            // starting position is genuinely known. Unlike a settings
            // toggle revealing a sub-setting as a direct, synchronous
            // reaction to the user's own click, this fetch resolves
            // asynchronously, after page load, so a placeholder would
            // reposition for a reason the user never triggered.
