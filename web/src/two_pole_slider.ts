...
export class TwoPoleSlider {
    ...
    constructor(options: TwoPoleSliderOptions) {
        ...
        // Move each tooltip into its own handle element rather than origin
        // (noUiSlider's own documented pattern for tooltip customization).
    ...
    private create_click_zones(breakpoint_count: number): void {
        ...
        for (let index = 0; index < breakpoint_count; index += 1) {
            ...
            // The start/end classes above override just the one side they
            // need via CSS cascade order, so this is only load-bearing for
            // middle zones.
