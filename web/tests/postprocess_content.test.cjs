...
run_test("collapsed media when thumbnails are hidden", ({override}) => {
    ...
    assert.equal(
        ...
        '<span class="message-media-collapsed-image" data-collapsed-image-html="<img alt=&quot;dinky-inline-image&quot; class=&quot;inline-image&quot; data-original-content-type=&quot;image/png&quot; data-original-dimensions=&quot;128x128&quot; data-original-src=&quot;/user_uploads/path/to/dinky-inline-image.png&quot; src=&quot;/user_uploads/thumbnail/path/to/dinky-inline-image.png/840x560.webp&quot;>"><span class="message-media-collapsed-image-link">dinky-inline-image</span><a role="button" tabindex="0" class="message-media-expand-button icon-button icon-button-square icon-button-neutral" aria-label="translated: Show preview"><i class="zulip-icon zulip-icon-expand" aria-hidden="true"></i></a></span>',
    );
    assert.equal(
        postprocess_content(
        ...
        '<span class="message-media-collapsed-image" data-collapsed-image-html="<img class=&quot;inline-image&quot; data-original-src=&quot;/user_uploads/path/to/my%20pic.png&quot; src=&quot;/user_uploads/thumbnail/path/to/my%20pic.png/840x560.webp&quot;>"><span class="message-media-collapsed-image-link">my pic.png</span><a role="button" tabindex="0" class="message-media-expand-button icon-button icon-button-square icon-button-neutral" aria-label="translated: Show preview"><i class="zulip-icon zulip-icon-expand" aria-hidden="true"></i></a></span>',
    );
    assert.equal(
        postprocess_content(
...
function build_message_card_embed_html(payload) {
    ...
    return (
        ...
        (payload.permalink ?? payload.fallback_link) +
        '" title="X - Post by jack (@jack)">X - Post by jack (@jack)</a></div>' +
...
run_test("message_card_embed_malformed_payload_is_ignored", () => {
    const result = postprocess_content(
        '<div class="message_embed" data-platform="twitter" data-message-card-embed="not valid json">' +
            ...
            '<div class="data-container">' +
            '<div class="message_embed_title"><a href="https://x.com/jack/status/20" title="X - Post by jack (@jack)">X - Post by jack (@jack)</a></div>' +
            '<div class="message_embed_description">placeholder</div>' +
            "</div>" +
            "</div>",
    );
