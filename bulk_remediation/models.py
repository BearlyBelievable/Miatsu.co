...
class BulkFieldRemediationJob(models.Model):
    ...
    # How long a job may run without a batch completion before it's
    # treated as abandoned. Set high relative to how quickly a healthy
    # queue should process a batch, so this should hopefully only fire
    # for a job that's actually stuck.
